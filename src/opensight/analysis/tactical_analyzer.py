...
class RoundPlay:
    ...
    utility_timing: dict[str, float] = field(
        default_factory=dict
    )  # when utilities were used (seconds)
...
class PlayerTacticalStats:
    ...
    # Positioning
    default_positions: dict[str, int] = field(
        default_factory=dict
    )  # where player usually positions
...
class TacticalAnalyzer:
    ...
    def _analyze_player_tactics(self) -> None:
        ...
        for steam_id, player_name in player_names.items():
            ...
            for play in self.round_plays:
                kills = [k for k in play.kills if k.get("attacker_steamid") == steam_id]
    ...
    def _extract_key_insights(self) -> list[str]:
        ...
        if self.round_plays:
            first_kill_times = [
                p.first_kill_time for p in self.round_plays if p.first_kill_time is not None
            ]
        ...
        if self.player_stats:
            strongest = max(self.player_stats.values(), key=lambda p: p.opening_kills)
            if strongest.opening_kills > 0:
                insights.append(f"{strongest.player_name} is the primary entry fragger")
