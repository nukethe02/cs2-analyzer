...
class PlayerBehaviorAnalyzer:
    ...
    def __init__(self, demo_data: DemoData):
        ...
        self._has_tick_data = (
            hasattr(demo_data, "ticks_df")
            and demo_data.ticks_df is not None
            and len(demo_data.ticks_df) > 0
        )
    ...
    def _extract_movement_samples(
        self, steamid: int, player_ticks: pd.DataFrame
    ) -> list[MovementSample]:
        ...
        for _, row in player_ticks.iterrows():
            ...
            # Get view angles
            view = (float(row.get("pitch", 0)), float(row.get("yaw", 0)))
    ...
    def _classify_movement(self, speed: float, vertical_vel: float, row: pd.Series) -> MovementType:
        ...
        for i in range(1, len(samples)):
            ...
            if direction != StrafeDirection.NONE:
                if current_strafe is None:
                    ...
                    # Continue strafe
                    current_strafe["max_speed"] = max(current_strafe["max_speed"], curr.speed)
    ...
    def _get_strafe_direction(self, prev: MovementSample, curr: MovementSample) -> StrafeDirection:
    ...
    def _finalize_strafe(self, strafe_data: dict, final_sample: MovementSample) -> StrafeAnalysis:
    ...
    def _analyze_counter_strafing(
        self, steamid: int, samples: list[MovementSample]
    ) -> CounterStrafeMetrics:
        """Analyze counter-strafing at engagement moments."""
        metrics = CounterStrafeMetrics()

        # Get kills by this player
        player_kills = [k for k in self.demo_data.kills if k.attacker_steamid == steamid]
        ...
        for kill in player_kills:
            ...
            if sample:
                ...
                stop_time = self._calc_time_to_stop(samples, kill_tick, tick_to_sample)
    ...
    def _analyze_positioning(
        ...
        for kill in player_kills:
            if (
                hasattr(kill, "attacker_x")
                and kill.attacker_x is not None
                and hasattr(kill, "victim_x")
                and kill.victim_x is not None
            ):
