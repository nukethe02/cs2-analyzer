...
def register_routes(app: Flask) -> None:
    ...
    def api_analyze():
        ...
        try:
            ...
            for steam_id, pm in sorted(metrics.items(), key=lambda x: x[1].overall_rating(), reverse=True):
                player_data = {
                    'name': pm.player_name,
                    'team': pm.team,
                ...
                if pm.engagement:
                    player_data.update({
                        ...
                        'kd': round(pm.engagement.total_kills / max(pm.engagement.total_deaths, 1), 2),
