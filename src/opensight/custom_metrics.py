...
class MetricVariable:
    ...
    default_value: Any = 0
    transform: Optional[str] = None  # Optional transform formula
...
class CustomMetric:
    ...
    benchmark_mid: Optional[float] = None
...
class CustomMetricBuilder:
    ...
    def create_metric(self, name: str, formula: str,
                      description: str = "",
                      metric_type: MetricType = MetricType.SCALAR,
                      aggregation: AggregationType = AggregationType.AVERAGE,
                      unit: str = "",
                      higher_is_better: bool = True,
                      benchmarks: Optional[tuple[float, float, float]] = None,
                      created_by: str = "") -> CustomMetric:
        """
        ...
        # Generate ID
        metric_id = hashlib.md5(f"{name}_{formula}_{datetime.now().isoformat()}".encode()).hexdigest()[:12]
    ...
    def _validate_formula(self, formula: str) -> tuple[bool, Optional[str], list[str]]:
        """
        ...
        try:
            test_vars = {name: 1.0 for name in variables_used}
    ...
    def update_metric(self, metric_id: str, **kwargs) -> Optional[CustomMetric]:
    ...
    def get_metric(self, metric_id: str) -> Optional[CustomMetric]:
        """Get a metric by ID."""
...
class MetricCalculator:
    ...
    def calculate(self, metric: CustomMetric,
                  player_stats: dict[str, Any]) -> MetricResult:
        """
...
_default_builder: Optional[CustomMetricBuilder] = None
