...
def _display_economy_metrics(data, steam_id: int | None, metrics_funcs: dict) -> None:
    ...
    for _sid, econ in sorted(
        econ_results.items(), key=lambda x: x[1].avg_equipment_value, reverse=True
    ):
        dmg_per_dollar = (
            f"{econ.damage_per_dollar:.2f}"
            if hasattr(econ, "damage_per_dollar") and econ.damage_per_dollar
            else "N/A"
        )
        table.add_row(
            econ.player_name,
...
def _display_utility_metrics(data, steam_id: int | None, metrics_funcs: dict) -> None:
    ...
    for _sid, util in sorted(
        utility_results.items(), key=lambda x: x[1].total_thrown, reverse=True
    ):
        table.add_row(
            util.player_name,
...
def _display_trade_metrics(data, steam_id: int | None, metrics_funcs: dict) -> None:
    ...
    for _sid, trade in sorted(trade_results.items(), key=lambda x: x[1].trade_rate, reverse=True):
        table.add_row(
            trade.player_name,
...
def _display_opening_metrics(data, steam_id: int | None, metrics_funcs: dict) -> None:
    ...
    for _sid, opening in sorted(opening_results.items(), key=lambda x: x[1].win_rate, reverse=True):
        table.add_row(
            opening.player_name,
            str(opening.wins),
            str(opening.losses),
