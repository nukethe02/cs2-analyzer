...
class TeamMoraleReport:
    ...
    # Team dynamics
    dominant_communicator: Optional[str] = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "timestamp": self.timestamp,
            "overall": {
                "morale": self.overall_morale.value,
                "score": round(self.morale_score, 2)
            },
            "communication": {
...
class SentimentAnalyzer:
    ...
    def analyze_message(self, message: VoiceMessage) -> VoiceMessage:
        ...
        if positive_total + negative_total > 0:
            message.sentiment_score = (positive_total - negative_total) / (positive_total + negative_total)
        else:
        ...
        message.is_constructive = (
            callout_score > 0 or
            strategy_score > 0 or
            encouragement_score > 0 or
            (positive_score > negative_score and toxic_score == 0)
        )
...
class CommunicationAnalyzer:
    ...
    def analyze_team_comms(self, messages: list[dict[str, Any]],
                           team_steamids: list[str],
                           demo_data: dict[str, Any]) -> TeamMoraleReport:
        """
        ...
        if analyzed_messages:
            ...
            report.toxic_incidents = len([m for m in analyzed_messages
                                          if m.sentiment == SentimentType.TOXIC])
        ...
        report.player_stats = self._calculate_player_stats(
            analyzed_messages, team_steamids
        )
    ...
    def _calculate_player_stats(self, messages: list[VoiceMessage],
                                team_steamids: list[str]) -> list[PlayerCommunicationStats]:
        ...
        for steamid, stats in stats_by_player.items():
            ...
            if player_messages:
                # Average sentiment
                stats.avg_sentiment = sum(m.sentiment_score for m in player_messages) / len(player_messages)
    ...
    def _build_morale_timeline(self, messages: list[VoiceMessage],
        ...
        for round_num in range(1, num_rounds + 1):
            ...
            timeline.append({
                ...
                "morale_trend": "up" if avg_sentiment > 0.1 else "down" if avg_sentiment < -0.1 else "stable"
            })
    ...
    def _identify_team_dynamics(self, report: TeamMoraleReport) -> None:
        ...
        if report.player_stats:
            sorted_by_comms = sorted(report.player_stats,
                                     key=lambda p: p.total_messages, reverse=True)
            if sorted_by_comms[0].total_messages > 0:
                report.dominant_communicator = sorted_by_comms[0].player_name

            # Find quiet players
            avg_messages = sum(p.total_messages for p in report.player_stats) / len(report.player_stats)
            report.quiet_players = [
                p.player_name for p in report.player_stats
                if p.total_messages < avg_messages * 0.3
            ]

        # Identify tension moments from timeline
        for i, round_data in enumerate(report.morale_timeline):
    ...
    def _generate_recommendations(self, report: TeamMoraleReport) -> list[str]:
        ...
        if report.callouts_per_round < 1.5:
            recommendations.append(
                "Callout frequency is low. Encourage more information sharing."
            )
        ...
        if report.quiet_players:
            ...
            recommendations.append(
                f"Encourage participation from quieter players: {names}"
            )
...
class VoiceTranscriber:
    ...
    def transcribe_demo_audio(self, demo_path: Path,
...
class ChatLogAnalyzer:
    ...
    def analyze_chat_log(self, chat_messages: list[dict[str, Any]],
                         team_steamids: list[str]) -> dict[str, Any]:
        """
...
_default_analyzer: Optional[CommunicationAnalyzer] = None
...
def analyze_team_morale(messages: list[dict[str, Any]],
                        team_steamids: list[str],
                        demo_data: dict[str, Any]) -> dict[str, Any]:
    """
...
def analyze_chat_log(chat_messages: list[dict[str, Any]],
                     team_steamids: list[str]) -> dict[str, Any]:
    """
